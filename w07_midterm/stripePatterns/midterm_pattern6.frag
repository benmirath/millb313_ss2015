...
void main(){
    ...
    float time_anim1 = time * 20.;

    // float timeFrag = fract (time);
    vec2 tileAmount = vec2 (15, 15);
    vec2 grid1 = tileOffset(st,  tileAmount);
    grid1 -= .5;
    grid1 *= rotationMatrix ((PI * 0.5));
    grid1 += .5;
    
    // grid1 = grid1 * rotationMatrix (PI);


    color = mix(col1, col2, circlePattern(grid1, 0.75 + (sin (time_anim1) * .1), 0.10 + (cos (time_anim1) * .05)));
