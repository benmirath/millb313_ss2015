...
float random (float _x) { return fract(sin(_x)*10e5); }
...
void main(){
    ...
    // }
    vec4 col1 = vec4 (vec3 (hsb2rgb ( vec3 (curRotation * 0.1, 1., 1.))), 1.);
    vec4 col2 = vec4 (vec3 (hsb2rgb ( vec3 (curRotationContra * 0.2, 1., 1.))), 1.);
    vec4 col3 = vec4 (vec3 (hsb2rgb ( vec3 (u_time * 0.05, 1., 1.))), 1.);
