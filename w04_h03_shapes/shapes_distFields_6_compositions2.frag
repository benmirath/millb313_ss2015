...
void main(){
    ...
    float pct = 0.0;
    // pct = distance(st,vec2(0.2)) + distance(st,vec2(0.7));
    // pct = distance(st,vec2(0.4)) * distance(st,vec2(0.6));
    pct = min(distance(st,vec2(0.6)),distance(st,vec2(0.6)));
    pct = max(distance(st,vec2(0.4)),distance(st,vec2(0.6)));
    // pct = pow(distance(st,vec2(0.4)),distance(st,vec2(0.6)));

    float posAnim = sin (u_time) * 0.25;
    pct = mix (
        min(distance(st,vec2(posAnim + 0.5)),distance(st,vec2(posAnim + 0.7 + sin (u_time)))), 
    ...
    vec3 color = mix (col1, col2, sin (u_time * 0.1));
